...
async function setupTestAgents() {
    ...
    await query("insert into agents (id, name, rate_per_1k_tokens, balance_lamports, pending_lamports) values ($1, $2, $3, $4, $5)", ["test_agent_a", "Test Agent A", 1000, 100000, 0]);
    // Create Agent B (callee, rate=2000)
    await query("insert into agents (id, name, rate_per_1k_tokens, balance_lamports, pending_lamports) values ($1, $2, $3, $4, $5)", ["test_agent_b", "Test Agent B", 2000, 50000, 0]);
    // Create Agent C (callee, rate=5000)
    await query("insert into agents (id, name, rate_per_1k_tokens, balance_lamports, pending_lamports) values ($1, $2, $3, $4, $5)", ["test_agent_c", "Test Agent C", 5000, 50000, 0]);
...
async function testExecutionWithBalance() {
    ...
    try {
        ...
        const expectedABalance = 100000 - costA;
...
async function testInsufficientBalance() {
    ...
    await query("insert into agents (id, name, rate_per_1k_tokens, balance_lamports) values ($1, $2, $3, $4)", ["test_poor_agent", "Poor Agent", 1000, 50] // Only 50 lamports
    );
    try {
...
async function testComposability() {
    ...
    await query("insert into agents (id, name, rate_per_1k_tokens, balance_lamports) values ($1, $2, $3, $4) on conflict (id) do update set balance_lamports = 100_000", ["test_comp_d", "Comp D", 1000, 100000]);
    await query("insert into agents (id, name, rate_per_1k_tokens, balance_lamports) values ($1, $2, $3, $4) on conflict (id) do update set balance_lamports = 100_000", ["test_comp_e", "Comp E", 1000, 100000]);
    ...
    try {
        ...
        log(`❌ Composability test failed: ${error.message}`, "red");
        throw error;
    }
}
async function testMetrics() {
    ...
    try {
        ...
        log(`❌ Metrics failed: ${error.message}`, "red");
        throw error;
    }
}
async function testSettlementEligibility() {
    ...
    try {
        ...
        log(`❌ Settlement check failed: ${error.message}`, "red");
        throw error;
    }
}
async function testImmutableLedger() {
    ...
    try {
        const result = await query("select caller_agent_id, callee_agent_id, tool_name, tokens_used, cost_lamports, rate_per_1k_tokens, created_at from tool_usage limit 5");
        ...
        result.rows.slice(0, 3).forEach((row, i) => {
            log(`  ${i + 1}. ${row.caller_agent_id} → ${row.callee_agent_id}: ${row.tool_name} (${row.tokens_used} tokens, ${row.cost_lamports} lamports)`, "cyan");
        });
        log(`✅ Immutable ledger working correctly`, "green");
    }
    catch (error) {
        log(`❌ Ledger check failed: ${error.message}`, "red");
        throw error;
    }
}
...
async function main() {
    ...
    try {
        ...
        try {
            ...
            log(`Cleanup error: ${cleanupError.message}`, "red");
            process.exit(1);
        }
    }
}
main();
//# sourceMappingURL=e2e-pricing.js.map