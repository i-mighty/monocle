 ...
 */
export function calculateCost(tokensUsed, ratePer1kTokens) {
    if (tokensUsed < 0) {
...
async function getAgent(agentId) {
    const result = await query("select id, rate_per_1k_tokens, balance_lamports, pending_lamports from agents where id = $1", [agentId]);
 ...
 */
export async function onToolExecuted(callerId, calleeId, toolName, tokensUsed) {
    // Validate inputs
    ...
    if (caller.balance_lamports < cost) {
        throw new Error(`Insufficient balance: ${callerId} has ${caller.balance_lamports} lamports, ` +
    ...
    try {
        ...
        await query(`insert into tool_usage (caller_agent_id, callee_agent_id, tool_name, tokens_used, rate_per_1k_tokens, cost_lamports)
       values ($1, $2, $3, $4, $5, $6)`, [callerId, calleeId, toolName, tokensUsed, callee.rate_per_1k_tokens, cost]);
        // Deduct from caller's balance
        await query("update agents set balance_lamports = balance_lamports - $1 where id = $2", [cost, callerId]);
        // Credit to callee's pending balance
        await query("update agents set pending_lamports = pending_lamports + $1 where id = $2", [cost, calleeId]);
        await query("COMMIT");
    }
    catch (error) {
 ...
 */
export async function settleAgent(agentId, sendTransaction) {
    const agent = await getAgent(agentId);
    const pending = agent.pending_lamports;
    if (pending < PRICING_CONSTANTS.MIN_PAYOUT_LAMPORTS) {
        throw new Error(`Pending balance (${pending}) below minimum payout (${PRICING_CONSTANTS.MIN_PAYOUT_LAMPORTS})`);
    }
    // Calculate fee
    ...
    const settlementResult = await query(`insert into settlements (from_agent_id, to_agent_id, gross_lamports, platform_fee_lamports, net_lamports, status)
     values ($1, $1, $2, $3, $4, 'pending')
     returning id, from_agent_id, to_agent_id, gross_lamports, platform_fee_lamports, net_lamports, status`, [agentId, pending, platformFee, payout]);
    ...
    try {
        ...
        await query("update settlements set status = 'failed' where id = $1", [settlement.id]);
    ...
    try {
        ...
        await query("update settlements set tx_signature = $1, status = 'confirmed' where id = $2", [txSignature, settlement.id]);
        // Clear pending balance
        await query("update agents set pending_lamports = 0 where id = $1", [agentId]);
        // Record platform revenue
        await query("insert into platform_revenue (settlement_id, fee_lamports) values ($1, $2)", [settlement.id, platformFee]);
        await query("COMMIT");
    }
    catch (error) {
        await query("ROLLBACK");
 ...
 */
export async function checkSettlementEligibility(agentId) {
    const agent = await getAgent(agentId);
...
export async function getAgentMetrics(agentId) {
    const agent = await getAgent(agentId);
    const usageResult = await query(`select count(*) as call_count, sum(cost_lamports) as total_spend
     from tool_usage where caller_agent_id = $1`, [agentId]);
    const earningsResult = await query(`select count(*) as call_count, sum(cost_lamports) as total_earned
     from tool_usage where callee_agent_id = $1`, [agentId]);
    return {
        agentId,
