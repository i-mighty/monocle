...
import { settleAgent, checkSettlementEligibility, getAgentMetrics, PRICING_CONSTANTS, } from "../services/pricingService.js";
...
router.get("/settlements/:agentId", apiKeyAuth, async (req, res) => {
    try {
        ...
        const { rows } = await query(`select id, from_agent_id, to_agent_id, gross_lamports, platform_fee_lamports, net_lamports,
...
router.post("/topup", apiKeyAuth, async (req, res) => {
    try {
        ...
        const result = await query("update agents set balance_lamports = balance_lamports + $1 where id = $2 returning id, balance_lamports, pending_lamports", [Math.floor(Number(amountLamports)), agentId]);
