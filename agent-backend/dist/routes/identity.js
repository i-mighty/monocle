...
router.post("/verify-identity", apiKeyAuth, async (req, res) => {
    try {
        ...
        const initialBalance = 1000000; // 1M lamports initial balance for testing
        try {
            // Upsert agent with pricing
            await query(`insert into agents (id, name, rate_per_1k_tokens, balance_lamports, pending_lamports, created_at)
         values ($1, $2, $3, $4, $5, now())
         on conflict (id) do update set
           rate_per_1k_tokens = $3,
           name = $2`, [agentId, `${firstName} ${lastName}`, rate, initialBalance, 0]);
            res.json({
                status: "verified",
                agent: {
                    id: agentId,
